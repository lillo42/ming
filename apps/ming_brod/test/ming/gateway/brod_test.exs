defmodule Ming.Gateway.BrodTest do
  @moduledoc """
  Tests for `Ming.Gateway.Brod`.

  Tests tagged `:brod` are integration tests against a live broker.
  """
  ...
  defp fetch_records(topic, offset \\ 0) do
    {:ok, {_high_watermark, records}} =
      :brod.fetch(brod_endpoints(), to_string(topic), 0, offset, %{max_wait_time: 5_000})
  ...
  describe "init/1" do
    test "starts a brod client plus one subscriber per subscription" do
      ...
      assert {:ok, {_flags, children}} = Brod.init(opts)
      assert length(children) == 3

      ids = Enum.map(children, & &1.id)
      assert Brod.client_name(name) in ids
      assert sub1 in ids
      assert sub2 in ids
    end
    ...
    test "raises when connection endpoints are missing" do
      opts = [name: unique_name(:brod_gateway), connection: []]

      assert_raise KeyError, fn -> Brod.init(opts) end
    end
  end

  describe "producer/0" do
    test "returns the producer module" do
      assert Brod.producer() == Producer
    end
  end
  ...
  describe "provision_infrastructure/1" do
    test ":assume provision does not touch the broker" do
      ...
      assert :ok = Brod.provision_infrastructure(opts)
    end
  end
  ...
  describe "provision_infrastructure/1 against a live broker" do
    ...
    test ":create creates the topic" do
      ...
      assert :ok = Brod.provision_infrastructure(opts)
      assert {:ok, _metadata} = :brod.get_metadata(brod_endpoints(), [to_string(topic)])
    end
    ...
    test ":validate fails for a missing topic" do
      ...
      assert {:error, _} = Brod.provision_infrastructure(opts)
    end
  ...
  describe "end-to-end" do
    ...
    test "supervisor starts, publishes and consumes a message" do
      ...
      Application.put_env(:ming, :brod_test_target_pid, self())

      on_exit(fn ->
        Application.delete_env(:ming, :brod_test_target_pid)
        :brod.delete_topics(brod_endpoints(), [to_string(topic)], 10_000)
      end)
      ...
      pid = start_supervised!({Brod, opts})
      ...
      gateway_config = [adapter: Brod, name: name]
    ...
    test "post/2 round-trips through a real command processor" do
      ...
      Application.put_env(:ming, :brod_e2e_pid, self())

      Application.put_env(:ming, BrodE2EProcessor,
        gateways: [
          [
            adapter: Brod,
      ...
      on_exit(fn ->
        Application.delete_env(:ming, :brod_e2e_pid)
        Application.delete_env(:ming, BrodE2EProcessor)
        :brod.delete_topics(brod_endpoints(), [to_string(topic)], 10_000)
      end)

      pid = start_supervised!(BrodE2EProcessor)
      assert Process.alive?(pid)

      assert :ok = BrodE2EProcessor.post(%{"id" => 1, "amount" => 42}, :e2e_order)
    ...
    defp boot_e2e_processor(publications, subscriptions, topics) do
      ...
      on_exit(fn ->
        ...
        Application.delete_env(:ming, BrodE2EProcessor)
        :brod.delete_topics(brod_endpoints(), topics, 10_000)
      end)
    ...
    test "requeued messages without a requeue routing key are not redelivered" do
      ...
      assert :ok = BrodE2EProcessor.post(%{"id" => 1}, :e2e_order)
    ...
    test "rejected messages are forwarded to the dead letter topic" do
      ...
      assert :ok = BrodE2EProcessor.post(%{"id" => 2}, :e2e_order)
    ...
    test "unacceptable messages are forwarded to the invalid message topic" do
      ...
      client = Brod.client_name(gateway_name)
    ...
    test "consumes records from foreign producers without CloudEvents headers" do
      ...
      assert :ok = Brod.provision_infrastructure(opts)
      start_supervised!({Brod, opts})

      client = Brod.client_name(name)
    ...
    test "concurrent posts all succeed" do
      ...
      results =
        1..20
        |> Task.async_stream(fn i -> BrodE2EProcessor.post(%{"i" => i}, :e2e_order) end,
          max_concurrency: 10
        )
...
defmodule BrodE2ERouter do
  @moduledoc false
  use Ming.Router

  register(:e2e_order, handler: BrodE2EHandler)
  register(:e2e_retry, handler: BrodE2EHandler)
  register(:e2e_dlq, handler: BrodE2EHandler)
end

defmodule BrodE2EProcessor do
  @moduledoc false
  use Ming.CommandProcessor, otp_app: :ming

  router(BrodE2ERouter)
end

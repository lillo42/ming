defmodule Ming.Gateway.Brod.ConsumerTest do
  @moduledoc """
  Unit tests for `Ming.Gateway.Brod.Consumer` (no broker required).
  """
  ...
  setup do
    ...
    on_exit(fn ->
      ...
      Application.delete_env(:ming, :brod_test_response)
    end)
  ...
  describe "init/2" do
    test "builds state from init_info and cb_config" do
      ...
      cb_config = [routing_key: :order_created, command_processor: TestBrodProcessor]
      ...
      assert state.command_processor == TestBrodProcessor
    ...
    test "raises when routing_key is missing" do
      ...
      assert_raise KeyError, fn ->
        Consumer.init(init_info, command_processor: TestBrodProcessor)
      end
    end
  end
  ...
  describe "handle_message/2" do
    test "dispatches to the command processor and acks" do
      ...
      assert opts[:metadata] == %{
               routing_key: :order_created,
               command_process: TestBrodProcessor
             }
    ...
    test "forwards unaccepted messages to the invalid message routing key and acks" do
      Application.put_env(:ming, :brod_test_response, {:ok, {:reject, :unaccepted}})
    ...
    test "handles unaccepted messages returned bare by a halted pipeline" do
      Application.put_env(:ming, :brod_test_response, {:reject, :unaccepted})

defmodule Ming.Gateway.Kafka.ProducerTest do
  @moduledoc """
  Integration tests for `Ming.Gateway.Kafka.Producer` against a live broker.
  """
    ...
    start_supervised!(%{
      ...
      start:
        {:brod, :start_link_client,
         [kafka_endpoints(), Kafka.client_name(gateway_name), [auto_start_producers: true]]}
    })
  ...
  defp fetch_records(topic, offset \\ 0) do
    {:ok, {_high_watermark, records}} =
      :brod.fetch(kafka_endpoints(), topic, 0, offset, %{max_wait_time: 5_000})
